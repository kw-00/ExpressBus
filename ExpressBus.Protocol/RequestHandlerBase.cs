...
public abstract class RequestHandlerBase
{
    ...
    public async Task HandleRequestAsync()
    {
        ...
        await Connection.ReceiveFullAsync(typeBuffer.WritableMemory).ConfigureAwait(false);
        var typeByte = typeBuffer.Memory.Span[0];
        typeBuffer.Dispose();

        // Read 4 bytes: message size (little-endian int32)
        var sizeBuffer = CreateBuffer(4);
        await Connection.ReceiveFullAsync(sizeBuffer.WritableMemory).ConfigureAwait(false);
        ...
        await Connection.ReceiveFullAsync(payload.WritableMemory).ConfigureAwait(false);
        var requestBytes = payload.WritableMemory;
        ...
        where T : struct, IByteSerializable<T>, IMessageSize
    {
        ...
        response.ToBytes(owner.WritableMemory);
        return owner;
    }

...
using ExpressBus.Protocol.ByteOperations;
...
public abstract class NotificationHandlerBase
{
    ...
    public async Task HandleNotificationAsync(Stream stream)
    {
        ...
        if (typeByte == BroadcastNotification.MessageTypeIdentifier)
            await HandleBroadcastNotificationAsync(BroadcastNotification.FromBytes(notificationBytes)).ConfigureAwait(false);
        else
            throw new FormatException($"Unknown MessageTypeIdentifier: 0x{typeByte:X2}");
    }

    /// <summary>
    /// Handles a broadcast notification from the server.
    /// </summary>
    /// <remarks>
    /// Override this method in a derived class to process incoming broadcast notifications.
    /// The notification payload has already been deserialized.
    /// </remarks>
    protected abstract Task HandleBroadcastNotificationAsync(BroadcastNotification notification);
}

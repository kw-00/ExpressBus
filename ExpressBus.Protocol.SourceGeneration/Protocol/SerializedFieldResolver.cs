...
/// </remarks>
internal static class SerializedFieldResolver
{
	/// <summary>
	...
	/// <summary>
	/// Resolves <c>[GenerateSerializedField("fieldName", typeof(FieldType))]</c> attributes from the struct.
	/// </summary>
	/// <remarks>
	...
	/// <see cref="MessageFieldInfo"/> objects.
	/// </remarks>
	public static IEnumerable<MessageFieldInfo> ParseSerializedFieldAttributes(
		SyntaxSymbol symbol,
		SemanticModel semanticModel)
	{
		var serializedFieldAttrs = symbol.Declaration.AttributeLists
			.SelectMany(list => list.Attributes)
			.Where(attr =>
			{
				var name = attr.Name.ToString();
				return name == "GenerateSerializedField" || name == "GenerateSerializedFieldAttribute";
			});

		foreach (var attr in serializedFieldAttrs)
		{
			var argList = attr.ArgumentList?.Arguments;
			if (argList == null || argList.Value.Count < 2)
				continue;

			// First argument: field name (string literal).
			...
			if (fieldName != null)
			{
				yield return new MessageFieldInfo(fieldName, underlyingType, enumType, enumType is not null, attr);
			}
		}
	}
}
